...
public class PerlinNoise : MonoBehaviour
{
    ...
    public void OnValidate() // Not working when in editor
    {
    ...
    public void GenerateNoiseMap()
    {
        ...
        for (int y = 0; y < grid.WorldHeight; ++y)
        {
            for (int x = 0; x < grid.WorldWidth; ++x)
            {
                ...
                {
                    float xCoord = x / settings.scale * frequency + offset * frequency; // z komentarza pod filmem
                    float yCoord = y / settings.scale * frequency + offset * frequency;

                    float perlinValue = Mathf.PerlinNoise(xCoord, yCoord) * 2 - 1; // * 2 - 1 so its sometimes negative and value doesnt go only up later
